  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(
        'https://devlinkdb-1.onrender.com/api/v1/users/register',
      ...
      // Show error message
      message.error(error.response?.data?.data || 'Failed to register. Please try again.');
      setFormState(prevState => ({
        ...prevState,
        error: error.response?.data?.data || 'Failed to register. Please try again.',
        successMessage: null,
      }));
    }
  };
